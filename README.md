...
- **Networking** — a full IPv4 stack built from the NIC up: PCI enumeration, an RTL8139 driver (interrupt-driven receive, DMA ring), Ethernet/ARP, IPv4 + UDP, a DHCP client, ICMP, and a DNS resolver, plus a ring-3 `SYS_RESOLVE` syscall so user programs can look up hostnames. CI boots with a NIC on QEMU's user-mode network and proves the whole path end to end — it ARP-resolves the gateway, obtains the `10.0.2.15` DHCP lease, ICMP-pings the gateway, and then the shell's `nslookup example.com` **resolves a hostname to an A record from ring 3** (capability-gated). No TCP is planned ([docs/NETWORKING.md](docs/NETWORKING.md))
