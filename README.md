...
- **Networking** — PCI device enumeration, an RTL8139 NIC driver (interrupt-driven receive, DMA ring buffer), and an Ethernet/ARP link layer. CI boots with a NIC on QEMU's user-mode network and ARP-resolves the SLIRP gateway — proving the whole path (PCI → driver → transmit → receive interrupt → ARP parse) works both directions. IPv4/UDP/DHCP/DNS are the in-progress follow-up ([docs/NETWORKING.md](docs/NETWORKING.md))
