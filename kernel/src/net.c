 ...
 * else routes via the gateway's MAC. */
static const uint8_t *udp_next_hop(const uint8_t *dip) {
...
static void udp_tx_drain(void) {
    while (udp_tx_tail != udp_tx_head) {
        ...
        uint8_t mac[ETH_ADDR_LEN];
        if (resolve_mac(udp_next_hop(e->dip), mac)) {
 ...
 * (net_rx demuxes to bound sockets; frames for nobody are dropped),
 * drain queued SENDTOs, then service a pending resolve. */
