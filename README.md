...
- **Networking** — PCI device enumeration, an RTL8139 NIC driver (interrupt-driven receive, DMA ring buffer), an Ethernet/ARP link layer, and an IPv4 + UDP stack with a DHCP client. CI boots with a NIC on QEMU's user-mode network, ARP-resolves the SLIRP gateway, and completes a full DHCP exchange to obtain the `10.0.2.15` lease — proving the path from PCI enumeration through the driver, IPv4 checksums, and UDP works both directions. ICMP + DNS are the in-progress follow-up; no TCP is planned ([docs/NETWORKING.md](docs/NETWORKING.md))
