...
static inline int rf_sin(uint32_t th) {
    return rf_sin_q15[(th >> 24) & 255];
}
static inline int rf_cos(uint32_t th) {
    return rf_sin_q15[((th >> 24) + 64) & 255];
}
...
static uint32_t rf_priority_core(uint32_t phase, uint32_t base_q16, uint64_t waited) {
    ...
    int align_q15 = (int)(((int64_t)c * rf_avg_cos_q15 + (int64_t)s * rf_avg_sin_q15) >> 15);
...
static void rf_update_field(void) {
    ...
    int64_t mag2 =
        (int64_t)rf_avg_cos_q15 * rf_avg_cos_q15 + (int64_t)rf_avg_sin_q15 * rf_avg_sin_q15;
...
static void rf_advance(void) {
    for (uint32_t pid = 0; pid < MAX_PROCESSES; pid++) {
        ...
        int coupling_q15 = (int)(((int64_t)rf_avg_sin_q15 * c - (int64_t)rf_avg_cos_q15 * s) >> 15);
...
static void rf_emit(const char *s) {
    early_console_write(s);
}
...
void resonant_fixed_boot_report(void) {
    ...
    for (uint32_t t = 0; t < RF_BENCH_STEPS; t++) {
        ...
        for (int i = 0; i < RF_BENCH_N; i++) {
            ...
            int coupling_q15 = (int)(((int64_t)avg_sin * c - (int64_t)avg_cos * s) >> 15);
            int32_t pull = (int32_t)(((int64_t)coupling_q15 * (int64_t)RF_KC) >> 15);
        ...
        rr_last[rr_pick] = t;
        rr_runs[rr_pick]++;
        g = t - raw_last[raw_best];
        if (g > raw_maxgap)
            raw_maxgap = g;
        raw_last[raw_best] = t;
        raw_runs[raw_best]++;
        g = t - aged_last[aged_best];
        if (g > aged_maxgap)
            aged_maxgap = g;
        aged_last[aged_best] = t;
        aged_runs[aged_best]++;
    }
    ...
    for (int i = 0; i < RF_BENCH_N; i++) {
        ...
        if (rr_runs[i] < rr_mn)
            rr_mn = rr_runs[i];
        if (raw_runs[i] > raw_mx)
            raw_mx = raw_runs[i];
        if (raw_runs[i] < raw_mn)
            raw_mn = raw_runs[i];
        if (aged_runs[i] > aged_mx)
            aged_mx = aged_runs[i];
        if (aged_runs[i] < aged_mn)
            aged_mn = aged_runs[i];
    }
    ...
    bool aged_worse =
        (aged_mn == 0) || (aged_mx * rr_mn > rr_mx * aged_mn) || (aged_maxgap > rr_maxgap);
    o = rf_put(b, 0, "SCHED: verdict raw resonant STARVES; aged resonant fairness ");
    o = rf_put(b, o,
               aged_worse ? "WORSE than rr (hypothesis refuted, reported honestly)"
